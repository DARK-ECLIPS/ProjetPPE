...
function adminMenu() {
  ...
  fetch('controllers/jsonRequest/userInfo.php').then(res => {
    res.json().then(data => {
      ...
      if (data.matter != 'Professeur' && data.matter != 'Receptionniste') {
        document.querySelector('.creneau').innerHTML += `
          <a href="index.php?OperaPPE=admin&Action=menu">
...
function creanauMenu(option, param) {
  if (option == "prof") {
    fetch('controllers/jsonRequest/creneau/prof.php').then(res => {
      res.json().then(data => {
    ...
    fetch('controllers/jsonRequest/classe.php').then(res => {
      res.json().then(data => {
    ...
    fetch('controllers/jsonRequest/creneau/matiere.php').then(res => {
      res.json().then(data => {
...
function reservationMenu(option, param) {
  if (option == "cours") {
    fetch('controllers/jsonRequest/reservation/cours.php').then(res => {
      res.json().then(data => {
        for (i = 0; i < data.response.length; i++) {
          // On ajoute pour chaque objet de data à la class <select>, où l'id de la balise select est egale à prof
          document.querySelector('.select select[id=prof]').innerHTML += `<option value="${data.response[i].pseudo}">${data.response[i].nom}.${data.response[i].prenom[0]} | ${data.response[i].pseudo}</option>`
    ...
    fetch('controllers/jsonRequest/matiere.php').then(res => {
      res.json().then(data => {
